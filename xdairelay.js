...
let web3;
if (
  relayHttpProvider.indexOf('localhost') >= 0 ||
  relayHttpProvider.indexOf('dai.poa.network') >= 0
) {
  web3 = new Web3(
    new HDWalletProvider(process.env.mnemonic, relayHttpProvider)
  );
} else {
  web3 = new Web3();
...
app.post('/link', async (req, res) => {
  ...
  let expiredClaim = await contracts.Links.methods
    .isClaimExpired(
      req.body.id,
      req.body.sig,
      req.body.claimHash,
  ...
  if (expiredClaim) {
    ...
    contracts.Links.methods
      .claim(req.body.id, req.body.sig, req.body.claimHash, req.body.dest)
      .send(
        {
