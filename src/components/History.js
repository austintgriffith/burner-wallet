...
export default class History extends React.Component {
  ...
  async poll() {
    ...
    let lastElement =
      theseTransactionsByAddress[theseTransactionsByAddress.length - 1];
    if (
      lastElement &&
      (!this.state.lastHash || this.state.lastHash != lastElement.hash)
    ) {
        ...
        if (savedPublicKey) {
          ...
          for (let t in theseTransactionsByAddress) {
            if (theseTransactionsByAddress[t].from == target) {
              ...
              let theTx = await this.props.web3.eth.getTransaction(
                theseTransactionsByAddress[t].hash
              );
              let rawText = '';
              if (theseTransactionsByAddress[t].data == ':wave:') {
                ...
                let hashOfPublicKey = this.props.web3.utils
                  .sha3(publicKey)
                  .toLowerCase();
                let isValid = hashOfPublicKey.indexOf(
                  theTx.from.toLowerCase().substring(2)
                );
                if (isValid > 0) {
  ...
  onKeyDown = (event: React.KeyboardEvent<HTMLDivElement>): void => {
    ...
    if (
      event.key === 'Enter' &&
      (this.state.newChat || this.state.newChatAmount)
    ) {
  ...
  render() {
            ...
            <div style={{ paddingTop: 10, paddingBottom: 10 }}>
              ...
              <div className="content ops row" style={{ position: 'relative' }}>
                ...
                >
                  ...
                  <span>{`${cleanTime(
                    (block - theseTransactionsByAddress[r].blockNumber) * 5
                  )} ago`}</span>
                </div>
                <div style={{ position: 'absolute', left: 0 }}>
                  <Blockies
                    seed={theseTransactionsByAddress[r].from}
                    scale={3}
                  />
                </div>
                <div
          ...
          txns.push(
            <div key={'tx' + r} style={{ paddingTop: 3, paddingBottom: 21 }}>
              ...
              <div
                ...
                style={{ position: 'relative', paddingTop: 3 }}
              >
                <div
                  style={{
                    position: 'absolute',
    ...
    if (this.state.sendingChat) {
      sendChatButton = (
        <button
          ...
          style={{ whiteSpace: 'nowrap', backgroundColor: '#666666' }}
        >
    ...
    if (this.state.waving) {
        ...
        <button
          ...
          onClick={() => {
            ...
            this.props.send(
              ...
              0,
              120000,
              this.props.web3.utils.utf8ToHex(':wave:'),
              (result) => {
                if (result && result.transactionHash) {
      ...
      waveButton = (
        <button
          className="btn btn-large w-100"
          style={{ whiteSpace: 'nowrap', backgroundColor: '#aaaaaa' }}
          onClick={() => {
            this.props.changeAlert({
              type: 'warning',
              message: i18next.t('history.metamask_error'),
            });
          }}
        >
          <Scaler config={{ startZoomAt: 400, origin: '50% 50%' }}>
          ...
          <div className="col-4 p-1">
            <div className="input-group">
              ...
              <input
                ...
                onChange={(event) =>
                  this.setState({ newChatAmount: event.target.value })
                }
              />
            </div>
          </div>
          <div className="col-6 p-1">{chatInput}</div>
          <div className="col-2 p-1">{sendChatButton}</div>
        </div>
      );
    } else {
      sendForm = (
        <div className="content ops row">
          ...
          <div className="col-8 p-1">{chatInput}</div>
          <div className="col-2 p-1">{sendChatButton}</div>
        </div>
      );
    }
    ...
    return (
      <div style={{ marginTop: 20 }}>
        <div className="content ops row">
            ...
            <CopyToClipboard text={target}>
              <button
                ...
                onClick={() =>
                  this.props.changeAlert({
                    type: 'success',
                    message: target + ' copied to clipboard',
                  })
                }
              >
                <Scaler config={{ startZoomAt: 400, origin: '50% 50%' }}>
          ...
          <div className="col-2 p-1" />
          <div className="col-4 p-1">{waveButton}</div>
        </div>
