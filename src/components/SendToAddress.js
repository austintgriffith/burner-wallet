...
export default class SendToAddress extends React.Component {
  constructor(props) {
    ...
    if (!startMessage) {
      ...
      cookie.save('sendToStartMessage', startMessage, {
        path: '/',
        maxAge: 60,
      });
    }
    ...
    if (window.location.pathname) {
      if (window.location.pathname.length == 43) {
        ...
        initialState = Object.assign(
          initialState,
          this.props.parseAndCleanPath(window.location.pathname)
        );
  ...
  canSend() {
    ...
    return (
      ...
      (this.state.amount > 0 || this.state.message)
    );
  }
  ...
  send = async () => {
    ...
    if (this.state.canSend) {
      ...
      if (!ERC20TOKEN && parseFloat(this.props.balance) <= 0) {
        ...
        !ERC20TOKEN &&
        parseFloat(this.props.balance) - 0.0001 <= parseFloat(amount)
      ) {
        let extraHint = '';
        if (
          !ERC20TOKEN &&
          parseFloat(amount) - parseFloat(this.props.balance) <= 0.01
        ) {
          extraHint = '(gas costs)';
        }
        this.props.changeAlert({
          ...
          message:
            'Not enough funds: ' +
            dollarDisplay(
              Math.floor((parseFloat(this.props.balance) - 0.0001) * 100) / 100
            ) +
            ' ' +
            extraHint,
        });
        ...
        ERC20TOKEN &&
        parseFloat(this.props.balance) < parseFloat(amount)
      ) {
        console.log('SO THE BALANCE IS LESS!');
        this.props.changeAlert({
          type: 'warning',
          message: 'Not enough tokens: $' + parseFloat(this.props.balance),
        });
      } else {
        ...
        this.props.send(toAddress, value, 120000, txData, (result) => {
          if (result && result.transactionHash) {
            ...
            if (
              this.props.scannerState &&
              this.props.scannerState.daiposOrderId
            ) {
      ...
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('send_to_address.error'),
      });
    }
  };
        ...
        >
          <div className="content row">
            <div className="form-group w-100">
                    ...
                    <div
                      ...
                      onClick={() =>
                        this.props.changeAlert({
                          type: 'success',
                          message:
                            toAddress.toLowerCase() + ' copied to clipboard',
                        })
                      }
                    >
              ...
              <label htmlFor="amount_input">
                {i18n.t('send_to_address.send_amount')}
              </label>
              <div className="input-group">
