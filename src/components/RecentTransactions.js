...
export default ({
  ...
  for (let r in recentTxs) {
    ...
    if (thisValue > 0.0) {
      ...
      if (count++ < max) {
        ...
        txns.push(
          <hr
            key={'ruler' + recentTxs[r].hash}
            style={{ color: '#DFDFDF', marginTop: 0, marginBottom: 7 }}
          />
        );
        //}
            ...
            >
              <div className="col-3" style={{ textAlign: 'center' }}>
                <i
                  ...
                  style={{ color: '#39e917', fontSize: 70, opacity: 0.7 }}
                />
              </div>
              <div
                className="col-3"
  ...
  if (txns.length > 0) {
    return <div style={{ marginTop: 30 }}>{txns}</div>;
  } else {
    return <span />;
  }
};
