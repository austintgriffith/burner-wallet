...
export default class Exchange extends React.Component {
    ...
    if (props.ERC20TOKEN && this.props.network == 'xDai') {
      try {
        ...
        dendaiContract = new this.props.web3.eth.Contract(
          require('../contracts/' + props.ERC20TOKEN + '.abi.js'),
          require('../contracts/' + props.ERC20TOKEN + '.address.js')
        );
        vendorContract = new this.props.web3.eth.Contract(
          require('../contracts/' + props.ERC20VENDOR + '.abi.js'),
          require('../contracts/' + props.ERC20VENDOR + '.address.js')
        );
  ...
  async poll() {
    ...
    if (this.state.gettingGas) {
      if (this.state.ethBalanceShouldBe) {
        console.log(
          'ethBalanceShouldBe:',
          parseFloat(this.state.ethBalanceShouldBe),
          ' needs to be less than ',
          parseFloat(this.props.ethBalance)
        );
        if (
          parseFloat(this.state.ethBalanceShouldBe) <
          parseFloat(this.props.ethBalance)
    ...
    if (
      false ||
      (parseFloat(this.props.daiBalance) > 0 &&
            ...
            onClick={() => {
              if (this.state.gettingGas) {
                this.props.changeAlert({
                  type: 'warning',
                  message: 'Already trying to fuel up via xDai->ETH',
                });
              } else if (
                this.props.network != 'xDai' &&
                this.props.network != 'Unknown'
              ) {
                this.props.changeAlert({
                  type: 'warning',
                ...
                this.setState({
                  gettingGas: true,
                  ethBalanceShouldBe: parseFloat(this.props.ethBalance) + 0.001,
                });
                axios
                  ...
                  .then((response) => {
                    if (
                      ...
                      let AMOUNTNEEDEDFORACOUPLETXS = Math.round(
                        gwei * (1111000000 * 10) * 201000
                      ); // idk maybe enough for a couple transactions?

                      console.log(
                        "let's move ",
                        AMOUNTNEEDEDFORACOUPLETXS,
                        'from',
                        this.props.xdaiBalance,
                        'to',
                        this.props.ethBalance
                      );

                      let gasInEth = this.props.web3.utils.fromWei(
                        '' + AMOUNTNEEDEDFORACOUPLETXS,
                        'ether'
                      );
                      console.log('gasInEth', gasInEth);
                      let gasInXDai =
                        Math.floor(this.props.ethprice * gasInEth * 100) / 100;

                      if (gasInXDai > 0.5) gasInXDai = 0.5;
                      if (this.props.xdaiBalance < gasInXDai)
                        gasInXDai = this.props.xdaiBalance - 0.005;
                      ...
                      if (this.props.network == 'xDai') {
                        try {
                          gasEmitterContract = new this.props.web3.eth.Contract(
                            ...
                            require('../contracts/Emitter.address.js')
                          );
                        } catch (e) {
                      ...
                      if (gasEmitterContract) {
                        let amountInWei = this.props.web3.utils.toWei(
                          '' + gasInXDai,
                          'ether'
                        );
                        ...
                        if (this.state.xdaiMetaAccount) {
                          ...
                          paramsObject.data = gasEmitterContract.methods
                            .goToETH()
                            .encodeABI();

                          console.log('TTTTTTTTTTTTTTTTTTTTTX', paramsObject);

                          this.state.xdaiweb3.eth.accounts
                            .signTransaction(
                              paramsObject,
                              this.state.xdaiMetaAccount.privateKey
                            )
                            .then((signed) => {
                              console.log('========= >>> SIGNED', signed);
                              this.state.xdaiweb3.eth
    ...
    if (this.props.ERC20TOKEN && dendaiContract) {
      let denDaiBalance = await dendaiContract.methods
        .balanceOf(this.state.daiAddress)
        .call();
      ...
      let maxWithdrawlAmount = await vendorContract.methods
        .allowance(this.state.daiAddress)
        .call();
      maxWithdrawlAmount = mainnetweb3.utils.fromWei(
        maxWithdrawlAmount,
        'ether'
      );
      ...
      if (this.state.xdaiToDendaiMode == 'withdrawing') {
        ...
        let percentDone = Math.min(
          100,
          (txAge * 100) / dendaiToxDaiEstimatedTime + 5
        );

        let xdaiBalanceShouldBe =
          parseFloat(this.state.xdaiBalanceShouldBe) - 0.0005;
        console.log(
          'watching for ',
          this.props.xdaiBalance,
          'to be ',
        ...
        if (this.props.xdaiBalance >= xdaiBalanceShouldBe) {
          this.setState({
            ...
            loaderBarStatusText: i18n.t('exchange.funds_transferred'),
        ...
        let percentDone = Math.min(
          100,
          (txAge * 100) / daiToxDaiEstimatedTime + 5
        );
        ...
        console.log(
          ...
          this.props.xdaiBalance,
          'to be less than ',
          this.state.xdaiBalanceShouldBe + 0.0005
        );
        if (
          this.props.xdaiBalance <=
          this.state.xdaiBalanceShouldBe + 0.00005
        ) {
          this.setState({
            loaderBarPercent: 100,
            loaderBarStatusText: i18n.t('exchange.funds_bridged'),
            loaderBarColor: '#62f54a',
          });
          setTimeout(() => {
            this.setState({
    ...
    if (this.state.daiToXdaiMode == 'withdrawing') {
      ...
      let percentDone = Math.min(
        100,
        (txAge * 100) / xdaiToDaiEstimatedTime + 5
      );
      ...
      console.log(
        ...
        'to be ',
        this.state.daiBalanceShouldBe - 0.0005
      );
    ...
    if (this.state.ethToDaiMode == 'withdrawing') {
      ...
      let percentDone = Math.min(
        100,
        (txAge * 100) / exchangeEstimatedTime + 5
      );
      ...
      console.log(
        ...
        this.props.ethBalance,
        'to be ',
        this.state.ethBalanceShouldBe - 0.001
      );
      if (
        parseFloat(this.props.ethBalance) >=
        this.state.ethBalanceShouldBe - 0.001
  ...
  sendDai() {
    if (
      parseFloat(this.props.daiBalance) < parseFloat(this.state.daiSendAmount)
    ) {
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('exchange.insufficient_funds'),
      });
    } else if (
      !this.state.daiSendToAddress ||
      !this.state.daiSendToAddress.length === 42
    ) {
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('exchange.invalid_to_address'),
      });
    } else if (!(parseFloat(this.state.daiSendAmount) > 0)) {
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('exchange.invalid_to_amount'),
      });
    } else {
      this.setState({
        ...
        daiBalanceShouldBe:
          ...
          parseFloat(this.state.daiSendAmount),
  ...
  canSendDai() {
    return (
      this.state.daiSendToAddress &&
      this.state.daiSendToAddress.length === 42 &&
      parseFloat(this.state.daiSendAmount) > 0 &&
      parseFloat(this.state.daiSendAmount) <= parseFloat(this.props.daiBalance)
    );
  }
      ...
      .then((response) => {
        if (
          ...
          if (this.state.mainnetMetaAccount) {
            ...
            paramsObject.data = this.props.daiContract.methods
              .transfer(
                destination,
            ...
            this.state.mainnetweb3.eth.accounts
              .signTransaction(
                paramsObject,
                this.state.mainnetMetaAccount.privateKey
              )
              .then((signed) => {
                console.log('========= >>> SIGNED', signed);
                this.state.mainnetweb3.eth
                  .sendSignedTransaction(signed.rawTransaction)
                  .on('receipt', (receipt) => {
                    console.log('META RECEIPT', receipt);
                    if (
            ...
            let metaMaskDaiContract = new this.props.web3.eth.Contract(
              this.props.daiContract._jsonInterface,
              this.props.daiContract._address
            );
            console.log(
              'CURRENT DAI CONTRACT YOU NEED TO GET ABI FROM:',
              this.props.daiContract
            );
            this.props.tx(
              metaMaskDaiContract.methods.transfer(
                destination,
                this.state.mainnetweb3.utils.toWei('' + amount, 'ether')
  ...
  sendXdai() {
    if (
      parseFloat(this.props.xdaiBalance) < parseFloat(this.state.xdaiSendAmount)
    ) {
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('exchange.insufficient_funds'),
      ...
      !this.state.xdaiSendToAddress ||
      !this.state.xdaiSendToAddress.length === 42
    ) {
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('exchange.invalid_to_address'),
      ...
      this.setState({
        ...
        xdaiBalanceShouldBe:
          parseFloat(this.props.xdaiBalance) -
          parseFloat(this.state.xdaiSendAmount),
        loaderBarColor: '#f5eb4a',
        loaderBarStatusText:
          'Sending $' +
          this.state.xdaiSendAmount +
          ' to ' +
          this.state.xdaiSendToAddress,
        loaderBarPercent: 0,
        loaderBarStartTime: Date.now(),
  ...
  canSendXdai() {
    console.log(
      'canSendXdai',
      this.state.xdaiSendToAddress,
      this.state.xdaiSendToAddress.length,
      parseFloat(this.state.xdaiSendAmount),
      parseFloat(this.props.xdaiBalance)
    );
    return (
      this.state.xdaiSendToAddress &&
      this.state.xdaiSendToAddress.length === 42 &&
      parseFloat(this.state.xdaiSendAmount) > 0 &&
      parseFloat(this.state.xdaiSendAmount) <=
        parseFloat(this.props.xdaiBalance)
    );
  }
  ...
  sendEth() {
    let actualEthSendAmount =
      parseFloat(this.state.ethSendAmount) / parseFloat(this.props.ethprice);
    ...
    if (parseFloat(this.props.ethBalance) < actualEthSendAmount) {
      ...
      !this.state.ethSendToAddress ||
      !this.state.ethSendToAddress.length === 42
    ) {
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('exchange.invalid_to_address'),
      ...
      this.setState({
        ...
        ethBalanceShouldBe:
          parseFloat(this.props.ethBalance) - actualEthSendAmount,
        loaderBarColor: '#f5eb4a',
        loaderBarStatusText: i18n.t('exchange.calculate_gas_price'),
        loaderBarPercent: 0,
        loaderBarStartTime: Date.now(),
      ...
      if (this.state.mainnetMetaAccount) {
        actualEthSendAmount = this.state.mainnetweb3.utils.toWei(
          '' + Math.round(actualEthSendAmount * 10000) / 10000,
          'ether'
        );
      }
  ...
  canSendEth() {
    ...
    return (
      this.state.ethSendToAddress &&
      this.state.ethSendToAddress.length === 42 &&
      actualEthSendAmount > 0 &&
      actualEthSendAmount <= parseFloat(this.props.ethBalance)
    );
  }
  transferEth(destination, call, amount, message, cb) {
    if (this.state.mainnetMetaAccount) {
      ...
      if (call) {
        this.props.tx(call, 240000, 0, amount, (receipt) => {
          if (receipt) {
        ...
        this.props.send(destination, amount, 240000, (receipt) => {
          if (receipt) {
  ...
  render() {
    ...
    let adjustedFontSize = Math.round(
      (Math.min(document.documentElement.clientWidth, 600) / 600) * 24
    );
    let adjustedTop =
      Math.round(
        (Math.min(document.documentElement.clientWidth, 600) / 600) * -20
      ) + 9;
    ...
    if (this.props.ERC20TOKEN) {
      if (
        ...
        xdaiToDendaiDisplay = (
          <div className="content ops row" style={{ position: 'relative' }}>
            <button
              style={{
                width: Math.min(100, this.state.loaderBarPercent) + '%',
                backgroundColor: this.state.loaderBarColor,
                color: '#000000',
              }}
              className="btn btn-large"
            />
        ...
        if (
          !this.state.xdaiMetaAccount &&
          (this.props.network != 'xDai' && this.props.network != 'Unknown')
        ) {
          xdaiToDendaiDisplay = (
            ...
            <div className="content ops row">
              ...
              <div className="col-3 p-1">
                <button
                  ...
                  onClick={async () => {
                    let amountOfxDaiToDeposit = this.state.xdaiweb3.utils.toWei(
                      '' + this.state.amount,
                      'ether'
                    );
                    console.log(
                      'Using DenDai contract to deposit ' +
                        amountOfxDaiToDeposit +
                        ' xDai'
                    );
                    ...
                    this.setState({
                      ...
                      loaderBarStatusText:
                        'Depositing xDai into ' + this.props.ERC20NAME + '...',
                      loaderBarPercent: 0,
                      loaderBarStartTime: Date.now(),
                      loaderBarClick: () => {
                        alert(i18n.t('exchange.go_to_etherscan'));
                    ...
                    if (this.state.xdaiMetaAccount) {
                      ...
                      paramsObject.data = this.state.vendorContract.methods
                        .deposit()
                        .encodeABI();
        ...
        if (
          ...
          if (
            !this.props.isAdmin &&
            (!this.props.isVendor || !this.props.isVendor.isAllowed)
          ) {
            extraWithdrawInfo = (
              <div className="content ops row" style={{ paddingTop: 10 }}>
                <div style={{ width: '100%', textAlign: 'center' }}>
                  Maximum withdrawal amount: $
                  {this.props.dollarDisplay(this.state.maxWithdrawlAmount)}
                </div>
                <div
              ...
              <div className="content ops row">
                ...
                <div className="col-3 p-1">
                  <button
                    ...
                    style={this.props.buttonStyle.primary}
                    onClick={async () => {
                      let amountOfxDaiToWithdraw = this.state.xdaiweb3.utils.toWei(
                        '' + this.state.amount,
                        'ether'
                      );
                      console.log(
                      ...
                      this.setState({
                        ...
                        loaderBarColor: '#3efff8',
                        loaderBarStatusText:
                          'Withdrawing ' + this.props.ERC20NAME + ' to xDai...',
                        loaderBarPercent: 0,
                        loaderBarStartTime: Date.now(),
                        loaderBarClick: () => {
                          alert(i18n.t('exchange.go_to_etherscan'));
                      ...
                      if (this.state.xdaiMetaAccount) {
                        ...
                        paramsObject.to = this.state.vendorContract._address;
                        paramsObject.data = this.state.vendorContract.methods
                          .withdraw('' + amountOfxDaiToWithdraw)
                          .encodeABI();
                        ...
                        this.props.tx(
                          this.props.contracts[this.props.ERC20VENDOR].withdraw(
                            '' + amountOfxDaiToWithdraw
                          ),
                          120000,
                          0,
          ...
          <div className="content ops row">
            <div className="col-6 p-1">
              ...
              >
                <Scaler config={{ startZoomAt: 400, origin: '50% 50%' }}>
                  <i className="fas fa-arrow-up" />
                  <span>xDai to {this.props.ERC20NAME}</span>
                </Scaler>
              </button>
            </div>
            ...
            <div className="col-6 p-1">
              ...
              >
                <Scaler config={{ startZoomAt: 400, origin: '50% 50%' }}>
                  <i className="fas fa-arrow-down" /> {this.props.ERC20NAME} to
                  xDai
                </Scaler>
              </button>
            </div>
      ...
      if (this.props.contracts) {
        link =
          ...
          this.props.contracts[this.props.ERC20TOKEN]._address +
          '/contracts';
      }
      ...
      if (!this.state.mainnetMetaAccount && this.props.network != 'Mainnet') {
        daiToXdaiDisplay = (
          <div className="content ops row" style={{ textAlign: 'center' }}>
            <div className="col-12 p-1">
              <span>Error: MetaMask network must be: </span>
              <span style={{ fontWeight: 'bold', marginLeft: 5 }}>Mainnet</span>
              <a
                href="#"
                onClick={() => {
                ...
                <div className="input-group">
                  ...
                  <div
                    ...
                    onClick={() => {
                      this.setState(
                        {
                          amount: Math.floor(this.props.daiBalance * 100) / 100,
                        },
                        () => {
                          this.setState({
                            canSendDai: this.canSendDai(),
                  ...
                  <div
                    ...
                    onClick={() => {
                      this.setState(
                        {
                          amount:
                            Math.floor((this.props.xdaiBalance - 0.01) * 100) /
                            100,
                        },
                        () => {
                          this.setState({
                ...
                onClick={() => {
                  ...
                  if (this.state.xdaiMetaAccount) {
                    ...
                    let paramsObject = {
                      ...
                      value: this.state.xdaiweb3.utils.toWei(
                        '' + this.state.amount,
                        'ether'
                      ),
                      gas: 120000,
                      ...
                      axios
                        ...
                        .then((response) => {
                          if (
                            ...
                            response.data.average =
                              response.data.average +
                              response.data.average * GASBOOSTPRICE;
                            let gwei =
                              Math.round(response.data.average * 100) / 1000;

                            console.log(gwei);

                            let IDKAMOUNTTOLEAVE =
                              gwei * (1111000000 * 2) * 201000; // idk maybe enough for a couple transactions?
            ...
            <div className="col-3 p-1">
              <button
                ...
                onClick={async () => {
                  ...
                  let uniswapContract = new webToUse.eth.Contract(
                    uniswapContractObject.abi,
                    uniswapContractObject.address
                  );
                  console.log(uniswapContract);

                  let amountOfEth = this.state.amount / this.props.ethprice;
                  amountOfEth = webToUse.utils.toWei(
                    '' + Math.round(amountOfEth * 10000) / 10000,
                    'ether'
                  );
                  console.log('amountOfEth', amountOfEth);

                  let output = await uniswapContract.methods
                    .getTokenToEthOutputPrice(amountOfEth)
                    .call();
                  output = parseFloat(output);
                  output = output - output * 0.0333;
                  console.log(
                    'Expected amount of DAI: ',
                    webToUse.utils.fromWei('' + Math.round(output), 'ether')
                  );

                  let currentBlockNumber = await webToUse.eth.getBlockNumber();
                  let currentBlock = await webToUse.eth.getBlock(
                    currentBlockNumber
                  );
                  ...
                  let amountOfChange = parseFloat(
                    webToUse.utils.fromWei('' + mintokens, 'ether')
                  );
                  ...
                  this.setState({
                    daiBalanceAtStart: this.props.daiBalance,
                    daiBalanceShouldBe:
                      parseFloat(this.props.daiBalance) + amountOfChange,
                  });

                  ///TRANSFER ETH
                  this.transferEth(
                    uniswapContract._address,
                    uniswapContract.methods.ethToTokenSwapInput(
                      '' + mintokens,
                      '' + deadline
                    ),
                    amountOfEth,
            ...
            <div className="col-3 p-1">
              <button
                ...
                onClick={async () => {
                  ...
                  let amountOfDai = webToUse.utils.toWei(
                    '' + this.state.amount,
                    'ether'
                  );
                  ...
                  let output = await uniswapContract.methods
                    .getEthToTokenOutputPrice(amountOfDai)
                    .call();
                  output = parseFloat(output);
                  output = output - output * 0.0333;
                  console.log(
                    'Expected amount of ETH: ',
                    output,
                    webToUse.utils.fromWei('' + Math.round(output), 'ether')
                  );
                  ...
                  let amountOfChange = parseFloat(
                    webToUse.utils.fromWei('' + mineth, 'ether')
                  );
                  console.log('ETH should change by ', amountOfChange);

                  let eventualEthBalance =
                    parseFloat(this.props.ethBalance) +
                    parseFloat(amountOfChange);
                  console.log(
                    '----- WATCH FOR ETH BALANCE TO BE ',
                    eventualEthBalance
                  );
                  ...
                  let approval = await this.props.daiContract.methods
                    .allowance(this.state.daiAddress, uniswapExchangeAccount)
                    .call();
                  ...
                  if (this.state.mainnetMetaAccount) {
                      ...
                      .then((response) => {
                        if (
                          ...
                          if (approval < amountOfDai) {
                            ...
                            paramsObject.to = this.props.daiContract._address;
                            paramsObject.data = this.props.daiContract.methods
                              .approve(uniswapExchangeAccount, '' + amountOfDai)
                              .encodeABI();

                            console.log(
                              'APPROVE TTTTTTTTTTTTTTTTTTTTTX',
                              paramsObject
                            );
                                ...
                                this.state.mainnetweb3.eth
                                  ...
                                  .on('receipt', async (receipt) => {
                                    ...
                                    if (
                                      ...
                                      let manualNonce = await this.state.mainnetweb3.eth.getTransactionCount(
                                        this.state.daiAddress
                                      );
                                      console.log(
                                        'manually grabbed nonce as ',
                                        manualNonce
                                      );
                                      paramsObject = {
                                        nonce: manualNonce,
                                      ...
                                      paramsObject.to =
                                        uniswapContract._address;
                                      paramsObject.data = uniswapContract.methods
                                        .tokenToEthSwapInput(
                                          '' + amountOfDai,
                                          '' + mineth,
                                          '' + deadline
                          ...
                          console.log(
                            'ERRORed RESPONSE FROM ethgasstation',
                            response
                          );
                        }
                      });
                    ...
                    if (approval < amountOfDai) {
                      ...
                      this.props.tx(
                        metaMaskDaiContract.methods.approve(
                          uniswapExchangeAccount,
                          '' + amountOfDai
                        ), //do 1000x so we don't have to waste gas doing it again
                        100000,
                        0,
                        0,
                        (receipt) => {
                          if (receipt) {
                            ...
                            this.props.tx(
                              uniswapContract.methods.tokenToEthSwapInput(
                                '' + amountOfDai,
                                '' + mineth,
                                '' + deadline
                              ),
    ...
    if (this.state.sendDai) {
            ...
            <div className="form-group w-100">
              <div className="form-group w-100">
                ...
                <input
                  ...
                  onChange={(event) =>
                    this.updateState('daiSendToAddress', event.target.value)
                  }
                />
              </div>
              <div>
                {this.state.daiSendToAddress &&
                  this.state.daiSendToAddress.length == 42 && (
                    <Blockies
                      seed={this.state.daiSendToAddress.toLowerCase()}
                      scale={10}
                    />
                  )}
              </div>
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  onChange={(event) =>
                    this.updateState('daiSendAmount', event.target.value)
                  }
                />
                <div
                  className="input-group-append"
              ...
              <button
                ...
                className={`btn btn-success btn-lg w-100 ${
                  this.state.canSendDai ? '' : 'disabled'
                }`}
      ...
      sendDaiButton = (
        <button
          className="btn btn-large w-100"
          style={{ backgroundColor: '#888888', whiteSpace: 'nowrap' }}
          onClick={() => {
            this.setState({ sendDai: false });
          }}
        >
    ...
    let fundByWyreButton = (
      <button
        ...
        disabled={buttonsDisabled}
        style={Object.assign({}, this.props.buttonStyle.secondary, {
          color: '#fff',
            ...
            <div className="form-group w-100">
              <div className="form-group w-100">
                ...
                <input
                  ...
                  onChange={(event) =>
                    this.updateState('ethSendToAddress', event.target.value)
                  }
                />
              </div>
              <div>
                {this.state.ethSendToAddress &&
                  this.state.ethSendToAddress.length == 42 && (
                    <Blockies
                      seed={this.state.ethSendToAddress.toLowerCase()}
                      scale={10}
                    />
                  )}
              </div>
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  onChange={(event) =>
                    this.updateState('ethSendAmount', event.target.value)
                  }
                />
                <div
                  className="input-group-append"
                          ...
                          this.setState(
                            {
                              ethSendAmount:
                                Math.floor(
                                  this.props.ethprice * adjustedEthBalance * 100
                                ) / 100,
                            },
                            () => {
                              this.setState({
              ...
              <button
                ...
                disabled={buttonsDisabled}
                className={`btn btn-success btn-lg w-100 ${
                  this.state.canSendEth ? '' : 'disabled'
                }`}
            ...
            <div className="form-group w-100">
              <div className="form-group w-100">
                ...
                <input
                  ...
                  onChange={(event) =>
                    this.updateState('xdaiSendToAddress', event.target.value)
                  }
                />
              </div>
              <div>
                {this.state.xdaiSendToAddress &&
                  this.state.xdaiSendToAddress.length == 42 && (
                    <Blockies
                      seed={this.state.xdaiSendToAddress.toLowerCase()}
                      scale={10}
                    />
                  )}
              </div>
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  onChange={(event) =>
                    this.updateState('xdaiSendAmount', event.target.value)
                  }
                />
                <div
                  className="input-group-append"
              ...
              <button
                ...
                disabled={buttonsDisabled}
                className={`btn btn-success btn-lg w-100 ${
                  this.state.canSendXdai ? '' : 'disabled'
                }`}
    ...
    return (
      <div style={{ marginTop: 30 }}>
        ...
        <div className="main-card card w-100">{daiToXdaiDisplay}</div>
        ...
        <div className="main-card card w-100">{ethToDaiDisplay}</div>
        ...
        {(window.location.hostname.indexOf('localhost') >= 0 ||
          window.location.hostname.indexOf('wyre.xdai.io') >= 0 ||
          window.location.hostname.indexOf('s.xdai.io') >= 0) && (
          <div
            className="send-to-address card w-100"
            style={{ marginTop: 20, borderBottom: 0, paddingTop: 50 }}
          >
            <div className="content ops row">
              <div className="col-2 p-1">
