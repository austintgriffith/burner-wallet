...
export default class SendToAddress extends React.Component {
  constructor(props) {
    ...
    initialState.metaAccount = tempweb3.eth.accounts.privateKeyToAccount(
      props.privateKey
    );
  ...
  async poll() {
    ...
    if (this.props.ERC20TOKEN) {
      fromBalance = await this.props.contracts[this.props.ERC20TOKEN]
        .balanceOf('' + this.state.fromAddress)
        .call();
    } else {
      fromBalance = await this.props.web3.eth.getBalance(
        '' + this.state.fromAddress
      );
    }

    fromBalance = parseFloat(
      this.props.web3.utils.fromWei(fromBalance, 'ether')
    );
  ...
  canWithdraw() {
    return (
      ...
      parseFloat(this.state.amount) <= parseFloat(this.state.fromBalance)
    );
  }
  ...
  withdraw = () => {
    ...
    if (this.state.canWithdraw) {
      ...
      const amountMinusFee = String(
        parseFloat(amount) - feeToDeductFromWithdrawl
      );
      ...
      if (this.props.ERC20TOKEN) {
        tx = {
          ...
          data: this.props.contracts[this.props.ERC20TOKEN]
            .transfer(
              this.props.address,
              this.props.web3.utils.toWei('' + amountMinusFee, 'ether')
            )
            .encodeABI(),
          gas: 60000,
      ...
      this.props.web3.eth.accounts
        .signTransaction(tx, metaAccount.privateKey)
        .then((signed) => {
          this.props.web3.eth
      ...
      this.props.changeAlert({
        type: 'warning',
        message: i18n.t('withdraw_from_private.error'),
      });
    }
  };
              ...
              <div className="col-12 p-1">
                <button
                  ...
                  onClick={() => {
                    ...
                    if (currentAmount)
                      currentAmount += parseFloat(costInDollars);
                ...
                >
                  <Scaler config={{ startZoomAt: 400, origin: '50% 50%' }}>
                    <span>{this.props.web3.utils.hexToUtf8(prod.name)}</span>
                    <span> {this.props.dollarDisplay(costInDollars)}</span>
                  </Scaler>
                </button>
              </div>
            </div>
          ...
          <div className="form-group w-100">
            <div className="form-group w-100">
              <label htmlFor="amount_input">
                {i18n.t('withdraw_from_private.from_address')}
              </label>
              <input
                readOnly
                type="text"
            ...
            <label htmlFor="amount_input">
              {i18n.t('withdraw_from_private.amount')}
            </label>
