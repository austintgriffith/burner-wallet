...
export default class YourModule extends React.Component {
  ...
  async pollInterval() {
    ...
    if (this.state && this.state.YourContract) {
      ...
      let yourContractBalance = await this.props.web3.eth.getBalance(
      ...
      yourContractBalance = this.props.web3.utils.fromWei(
        yourContractBalance,
        "ether"
      );
      this.setState({
  ...
  clicked(name) {
    ...
    */
    this.props.tx(
      this.state.YourContract.updateVar(name),
      120000,
      0,
      0,
      result => {
  ...
  deployYourContract() {
    ...
    this.props.tx(
      this.state.YourContract._contract.deploy({ data: code }),
      640000,
      receipt => {
        let yourContract = this.props.contractLoader(
          "YourContract",
  ...
  render() {
    if (!this.state.YourContract) {
      return <div>LOADING YOURCONTRACT...</div>;
    }
      ...
      <div>
        <div className="form-group w-100">
          <div style={{ width: "100%", textAlign: "center" }}>
            ...
            <div>
              xdaiweb3 is on block {this.state.xdaiBlockNumber} and version{" "}
              {this.props.xdaiweb3.version}
            </div>
            <div>
