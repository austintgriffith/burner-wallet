  ...
  async poll() {
    ...
    if (this.state.vendor) {
      if (!this.state.vendorObject) {
        let vendorData = await this.props.contracts[this.props.ERC20VENDOR]
      ...
      let found = true;
      while (found) {
        let nextProduct = await this.props.contracts[this.props.ERC20VENDOR]
          .products(this.state.vendor, id)
          .call();
    ...
    if (vendor) {
      ...
      for (let p in correctProducts) {
        ...
        if (prod.exists && prod.isAvailable) {
              ...
              >
                <div className="content qr row">
                  ...
                  <div style={{ width: '100%', textAlign: 'center' }}>
                    <div>{correctVendorObject.name}</div> {theName}: $
          ...
          let available = <i className="far fa-eye" />;
          if (!prod.isAvailable) {
            available = <i className="far fa-eye" style={{ opacity: 0.3 }} />;
          }

          products.push(
            <div
              className="content bridge row"
              style={{
            ...
            >
              ...
              <div className="col-4 p-1">{theName}</div>
              <div className="col-3 p-1">${dollarDisplay(theAmount)}</div>
              <div className="col-4 p-1">
                <button
                  ...
                  onClick={() => {
                    this.setState(
                      { loading: true, products: false, vendor: false },
                      () => {
