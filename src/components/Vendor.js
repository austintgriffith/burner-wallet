...
export default class Advanced extends React.Component {
  ...
  render() {
      ...
      if (prod.exists) {
        ...
        products.push(
          <div key={p} className="content bridge row" style={{ opacity }}>
            <div className="col-6 p-1">{web3.utils.hexToUtf8(prod.name)}</div>
            <div className="col-4 p-1">
              {dollarDisplay(web3.utils.fromWei(prod.cost, 'ether'))}
            </div>
            <div className="col-2 p-1">{productIsActive}</div>
          </div>
        );
      }
    }
    ...
    if (this.state.changingActive) {
      venderButtonText = (
        <div>
          <i className="fas fa-cog fa-spin" /> {i18n.t('vendor.updating')}
        </div>
      );
    } else if (vendor.isActive) {
      venderButtonText = (
        <div>
          <i className="fas fa-thumbs-up" /> {i18n.t('vendor.open')}
        </div>
      );
    } else {
      venderButtonText = (
        <div>
          <i className="fas fa-thumbs-down" /> {i18n.t('vendor.closed')}
        </div>
      );
    }
