...
class App extends Component {
  ...
  async ensLookup(name) {
    ...
    let ensResolver = new this.state.mainnetweb3.eth.Contract(
      require('./contracts/ENSResolver.abi.js'),
      resolver
    );
  ...
  async chainClaim(tx, contracts) {
    ...
    if (parseInt(fund[5].toString()) > 0) {
      ...
      let sig = this.state.web3.eth.accounts.sign(
        claimHash,
        this.state.claimKey
      );
      sig = sig.signature;
      ...
      if (FAILCOUNT++ > 1) {
        this.changeAlert({
          type: 'danger',
          message: 'Sorry. Failed to claim. Already claimed?',
        });
        setTimeout(() => {
          this.setState({ sending: false }, () => {
            //alert("DONE")
  ...
  async relayClaim() {
    ...
    let fund = await this.state.contracts.Links.funds(
      this.state.claimId
    ).call();
    if (parseInt(fund[5].toString()) > 0) {
      this.setState({ fund: fund });
      ...
      console.log(
        'Calling encodeABU on Links.claim() ',
        this.state.claimId,
        sig,
        claimHash,
        this.state.account
      ...
      console.log(
        'Hitting relayClient with relayTransaction()',
        claimData,
        options
      );
...
*/
  ...
  initRecentTxs() {
    ...
    if (Object.keys(transactionsByAddress).length === 0) {
      transactionsByAddress = localStorage.getItem(
        this.state.account + 'transactionsByAddress'
      );
      try {
  ...
  sortAndSaveTransactions(recentTxs, transactionsByAddress) {
    ...
    localStorage.setItem(
      this.state.account + 'recentTxs',
      JSON.stringify(recentTxs)
    );
    localStorage.setItem(
      this.state.account + 'transactionsByAddress',
      JSON.stringify(transactionsByAddress)
    );

    this.setState(
      { recentTxs: recentTxs, transactionsByAddress: transactionsByAddress },
      () => {
        if (ERC20TOKEN) {
  ...
  render() {
        ...
        <div>
          <ContractLoader
            ...
            onReady={(contracts, customLoader) => {
              ...
              this.setState(
                { contracts: contracts, customLoader: customLoader },
                async () => {
    ...
    if (this.state.extraHeadroom) {
      extraHead = <div style={{ marginTop: this.state.extraHeadroom }} />;
    }
                    ...
                    eventParser = (
                      <div style={{ color: '#000000' }}>
                        ...
                        <Events
                          ...
                          onUpdate={(eventData, allEvents) => {
                            this.setState(
                              { transferFromWithData: allEvents },
                              this.syncFullTransactions
                            );
                          }}
                        />
                        ...
                        <Events
                          ...
                          onUpdate={(vendor, all) => {
                            ...
                            if (
                              ...
                              vendors[vendor.vendor].blockNumber <
                                vendor.blockNumber
                            ) {
                              vendors[vendor.vendor] = {
                                name: this.state.web3.utils.hexToUtf8(
                                  vendor.name
                                ),
                ...
                if (view.indexOf('account_') == 0) {
                  ...
                  return (
                    <div>
                      ...
                      >
                        <NavCard
                          title={<div>{i18n.t('history_chat')}</div>}
                      ...
                      <div>
                        ...
                        >
                          ...
                          <Advanced
                            ...
                            changeAlert={this.changeAlert}
                            goBack={this.goBack.bind(this)}
                            setPossibleNewPrivateKey={this.setPossibleNewPrivateKey.bind(
                              this
                            )}
                          />
                        </div>
                  ...
                  case 'send_to_address':
                    return (
                      <div>
                        ...
                        >
                          <NavCard
                            title={i18n.t('send_to_address_title')}
                            goBack={this.goBack.bind(this)}
                          />
                          {defaultBalanceDisplay}
                          <SendToAddress
                  ...
                  case 'request_funds':
                    return (
                      <div>
                        ...
                        >
                          <NavCard
                            title={i18n.t('request_funds_title')}
                            goBack={this.goBack.bind(this)}
                          />
                          {defaultBalanceDisplay}
                          <RequestFunds
                        ...
                        >
                          ...
                          <SendWithLink
                            ...
                            sendWithLink={(amount, cb) => {
                              let randomHash = this.state.web3.utils.sha3(
                                '' + Math.random()
                              );
                              ...
                              this.state.tx(
                                ...
                                async (receipt) => {
                                  this.setState(
                                    {
                                      sendLink: randomHash,
                                      sendKey: randomWallet.privateKey,
                                    },
                                    () => {
                          ...
                          <BurnWallet
                            ...
                            burnWallet={() => {
                              ...
                              if (
                                ...
                                localStorage.setItem(
                                  this.state.account + 'metaPrivateKey',
                                  ''
                                );
                                localStorage.setItem(
                                  this.state.account + 'recentTxs',
                  ...
                  if (!this.state.parsingTheChain) {
                    this.setState({ parsingTheChain: true }, async () => {
                      ...
                      if (
                        ...
                        if (!loadedBlocksTop) {
                          loadedBlocksTop = localStorage.getItem(
                            this.state.account + 'loadedBlocksTop'
                          );
                        }
          ...
          render={({ state, fullpageApi }) => {
              ...
              <div>
                <div className="content ops row">
                  ...
                  <div
                    ...
                    onClick={() => {
                      fullpageApi.moveSectionUp();
                      this.changeView('send_with_link');
                    }}
                  >
                    <button
                      className="btn btn-large w-100"
                ...
                <div>
                  <div className="content ops row">
                    ...
                    >
                      ...
                      >
                        <Scaler
                          config={{ startZoomAt: 400, origin: '50% 50%' }}
                        >
                          <i className="fas fa-share" /> Share
                        </Scaler>
                      </button>
            ...
            let actualQR = (
              <div style={{ width: 345, height: 345, overflow: 'hidden' }}>
                <img
                  src={burnerloader}
                  style={{ width: 566, height: 340, marginLeft: -116 }}
                />
              </div>
            );
            if (this.state.account) {
              fullOpacity = 1;
              actualQR = <QRCode value={this.state.account} size={325} />;
              loaderOverlay = '';
            }
            ...
            return (
              <ReactFullpage.Wrapper>
                ...
                >
                  ...
                  <div style={{ opacity: fullOpacity }}>{fulldollaamount}</div>
                  <div
                    style={{
                      opacity: fullOpacity * 2,
                      position: 'absolute',
                      top: '90%',
                      transform: 'scale(3)',
                      left: '50%',
                ...
                <div className="section">{mainbody}</div>
        ...
        this.state.web3.eth
          ...
          .on('error', (error) => {
            ...
            if (errorString.indexOf('have enough funds') >= 0) {
              this.changeAlert({
                type: 'danger',
                message: 'Not enough funds to send message.',
              });
            } else {
