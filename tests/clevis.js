...
const clevisConfig = JSON.parse(
  fs
    .readFileSync('clevis.json')
    .toString()
    .trim()
);
...
function localContractAbi(contract) {
  return JSON.parse(
    fs
      .readFileSync(
        clevisConfig.CONTRACTS_FOLDER + '/' + contract + '/' + contract + '.abi'
      )
      .toString()
      .trim()
  );
...
module.exports = {
  ...
  contracts: fs
    .readFileSync(clevisConfig.ROOT_FOLDER + '/contracts.clevis')
    .toString()
    .trim()
    .split('\n'),
  reload: () => {
    describe('#reload() ', function() {
      it('should force browser to reload', async function() {
        fs.writeFileSync(
          clevisConfig.CRA_FOLDER + '/../public/reload.txt',
          Date.now()
        );
      });
    });
  ...
  compile: (contract) => {
    describe('#compile() ' + contract.magenta, function() {
      it(
        'should compile ' + contract.magenta + ' contract to bytecode',
        async function() {
          this.timeout(90000);
          ...
          assert(
            ...
            'No compiled contacts found.'
          );
          let count = 0;
          for (let c in result.contracts) {
            console.log(
              '\t\t' + 'contract ' + c.blue + ': ',
              result.contracts[c].bytecode.length
            );
            if (count++ == 0) {
              assert(
                result.contracts[c].bytecode.length > 1,
                'No bytecode for contract ' + c
              );
            }
          }
        }
  ...
  deploy: (contract, accountindex) => {
    describe('#deploy() ' + contract.magenta, function() {
      it(
        'should deploy ' + contract.magenta + ' as account ' + accountindex,
        async function() {
          this.timeout(360000);
      ...
      it('should inject contract address and abi into web app', async function() {
        ...
        for (let c in module.exports.contracts) {
          ...
          let abi = fs
            ...
            .trim();
          fs.writeFileSync(
            clevisConfig.CRA_FOLDER + '/contracts/' + thisContract + '.abi.js',
            'module.exports = ' + abi
          );
          let bytecode = fs
            .readFileSync(
        ...
        fs.writeFileSync(
          clevisConfig.CRA_FOLDER + '/contracts/contracts.js',
          'module.exports = ' + JSON.stringify(module.exports.contracts)
        );
      ...
      it('should set Relay Hub address to Relay Recepient and provide it with a deposit', async function() {
        ...
        */
        ...
        result = await clevis(
          ...
          '0',
          relayHubAddress
        ); ///<<<-------- change this to the xDai relay hub!
        printTxResult(result);
  ...
  metamask: () => {
    describe('#transfer() ', function() {
      it('should give metamask account some ether or tokens to test', async function() {
        ...
        ); ///<<<-------- change this to your metamask accounts
        printTxResult(result);
        ...
        result = await clevis(
          ...
          '0.10',
          '0',
          '0xa9887c69538ccf8692066e0fd5d12a9593ab0403'
        ); ///<<<-------- change this to your metamask accounts
        printTxResult(result);
        result = await clevis(
          'send',
          '0.10',
          '0',
          '0x3baa2f6302ce57eb2ad94c9571b19e1073b079be'
        ); ///<<<-------- change this to your metamask accounts
        printTxResult(result);
        ...
        result = await clevis(
          ...
          '0.10',
          '0',
          '0x3606affd391d37a0f413c261bd3caf573c6360e1'
        ); ///<<<-------- change this to your metamask accounts
        printTxResult(result);

        result = await clevis(
          'contract',
          'mintNextTokenWithTokenURI',
          'Badges',
          '0',
          '0x34aA3F359A9D614239015126635CE7732c18fDF3',
          'https://badges.xdai.io/ethdenver/v1/json/burner.json'
        );
        printTxResult(result);
        result = await clevis(
          'contract',
        ...
        result = await clevis(
          ...
          '0',
          '0x34aA3F359A9D614239015126635CE7732c18fDF3',
          'https://badges.xdai.io/ethdenver/v1/json/buffidai.json'
        );
        printTxResult(result);
        result = await clevis(
          'contract',
        ...
        result = await clevis(
          ...
          'Badges',
          '0',
          '0xa0ddb01deab4f240cf6dad304e50b58085055372',
          'https://badges.xdai.io/ethdenver/v1/json/buffalo.json'
        );
        printTxResult(result);
        ...
        result = await clevis(
          ...
          '0',
          '0xa0ddb01deab4f240cf6dad304e50b58085055372',
          'https://badges.xdai.io/ethdenver/v1/json/gitcoin.json'
        );
        printTxResult(result);
        result = await clevis(
          'contract',
          ...
          '0',
          '0xa0ddb01deab4f240cf6dad304e50b58085055372',
          'https://badges.xdai.io/ethdenver/v1/json/consensys.json'
        );
        printTxResult(result);
  ...
  updateVendingMachineAddress: () => {
    describe('#transfer() ', function() {
      it('update vending machine address', async function() {
        this.timeout(600000);
        let vendingMachineAddress = require('fs')
          .readFileSync('./contracts/VendingMachine/VendingMachine.address')
          .toString()
          .trim();
        console.log(
          'changeVendingMachine to vendingMachineAddress:',
  ...
  full: () => {
    ...
    describe(bigHeader('RELAY HUB'), function() {
      it('should set Relay Hub address to Relay Recepient and provide it with a deposit', async function() {
        ...
        let links = new web3ForStake.eth.Contract(linksAbi, linksAddress, {
          from: accounts[0],
        });
        await links.methods.set_hub(relayHubAddress).send();
        await links.methods
          .deposit_to_relay_hub()
          .send({ value: web3ForStake.utils.toWei('0.5', 'ether') });
      });
    });
  },
...
let checkContractDeployment = async (contract) => {
  ...
  const address = await clevis(
    'contract',
    'getContract',
    'Example',
    web3.utils.fromAscii(contract)
  );
  console.log(
    tab,
    contract.blue +
      ' contract address is ' +
      (localAddress + '').magenta +
      ' deployed as: ' +
      (address + '').magenta
  );
